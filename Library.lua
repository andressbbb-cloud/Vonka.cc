...
local Library do
    ...
    Library = {
        ...
        Folders = {
            Directory = "Vonka.cc",
            Configs = "Vonka.cc/Configs",
            Assets = "Vonka.cc/Assets"
        },
    ...
    local Instances = { } do
        ...
        Instances.Tween = function(self, Info, Goal)
            ...
            -- Direct property set (zero overhead, no TweenService)
            for Property, Value in Goal do
    ...
    do
        local Tooltip = { Visible = false, Text = "" }
        ...
        local function UpdateTooltipPosition()
            ...
            local Body = Items["Body"].Instance

            local viewport = Camera.ViewportSize
            local mx, my = Mouse.X, Mouse.Y
            local pad = 14

            Root.Position = UDim2New(0, mx + pad, 0, my + pad)
            task.defer(function()
                if not Tooltip.Visible then
                    return
                end
                ...
                if y + size.Y > viewport.Y - 4 then
                    y = my - size.Y - pad
                end
                Root.Position = UDim2New(0, x, 0, y)
            end)
        end
        ...
        function Library:BindTooltip(GuiObject, Text)
            if not GuiObject or type(Text) ~= "string" or Text == "" then
                return
            end

            local entering = false
            self:Connect(GuiObject.MouseEnter, function()
                entering = true
            ...
            self:Connect(GuiObject.MouseLeave, function()
                entering = false
                Library:HideTooltip()
            end)
        end
    ...
    Library.GetTooltip = function(self, Data)
        if type(Data) ~= "table" then
            return nil
        end
        return Data.Tooltip or Data.tooltip or Data.Tip or Data.tip or Data.Description or Data.description
    end
    ...
    Library.Window = function(self, Data)
        ...
        local Items = { } do 
            ...
            local TitleText = Window.Name
            local RichTitle = StringGSub(TitleText, "(.*)(%.cc)(.*)", function(pre, cc, post)
                return pre .. '<font color="rgb(255,0,0)">' .. cc .. '</font>' .. post
            end)
            Items["Title"] = Instances:Create("TextLabel", {
                Parent = Items["MainFrame"].Instance,
                FontFace = Library.Font,
